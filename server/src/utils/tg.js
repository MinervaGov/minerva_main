...
async function notifyNewProposalTG(users, snapshotSpaceId, proposalId, aiSummary) {
    if (!users) {
        return
    }
    ...
    for (let agent in users) {
        ...
        for (let user in agent.usersTg) {
            ...
            bot.api.sendMessage(user, `📢*NEW PROPOSAL ALERT*📢\n\nAgent: *${agentName}*\n\n${aiSummary}\n\nRead more: [Snapshot](${proposalLink})`, {parse_mode: "Markdown"})
        }
    }
...
async function notifyDecisionTG(users, title, choices, aiResponse) {
    ...
    for (let agent in users) {
        ...
        for (let user in agent.usersTg) {
            ...
            bot.api.sendMessage(user, `✅*AGENT ${agentName} VOTED✅\n\nTitle: ${title}\n\nVote: ${choices[parseInt(aiResponse.vote)]}\n\nReason: {aiResponse.reason}`, {parse_mode: "Markdown"})
        }
    }
}
